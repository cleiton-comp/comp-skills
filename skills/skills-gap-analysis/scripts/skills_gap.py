...
def analyze(inventory, target, cols_inv, cols_tgt):
    ...
    if not open_gaps:
        ...
        top = open_gaps[0]
        insights.append(f"Maior prioridade: <strong>{top['skill']}</strong> (criticidade {top['criticality']}, gap {top['gap']}) → recomendação {top['recommendation']}.")
...
document.getElementById('hm-tbody').innerHTML = DATA.heatmap.map(h => {
  ...
  return `<tr><td><strong>${h.skill}</strong></td>${cells}</tr>`;
}).join('');

document.getElementById('gap-tbody').innerHTML = DATA.gaps.map(g =>
  `<tr><td><strong>${g.skill}</strong></td><td>${g.required}</td><td>${g.criticality}</td><td>${g.headcount_needed}</td><td>${g.covered}</td><td class="${g.gap > 0 ? 'text-rose-600 font-bold' : ''}">${g.gap}</td><td>${g.coverage_pct}%</td><td>${g.priority}</td><td><span class="pill rec-${g.recommendation}">${g.recommendation}</span></td><td class="text-slate-500 text-xs">${g.rationale}</td></tr>`).join('');
