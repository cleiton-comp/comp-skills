...
document.getElementById('priorities').innerHTML = DATA.priorities.map((p, i) => {
  const krs = (p.key_results || []).map(k => {
    if (typeof k === 'string') return `<div class="kr"><span>${k}</span></div>`;
    const tgt = k.target ? `<span class="kr-target">${k.target}</span>` : '';
    return `<div class="kr"><span>${k.kr || ''}${k.metric ? ` <span class="text-slate-400 text-xs">(${k.metric})</span>` : ''}</span>${tgt}</div>`;
  }).join('');
  const inits = (p.initiatives || []).length
    ? `<div class="mt-2 text-sm"><strong>Iniciativas:</strong><ul>${p.initiatives.map(x => `<li>${x}</li>`).join('')}</ul></div>` : '';
  return `<div class="priority">
    <div class="flex justify-between items-start gap-3 mb-1">
      <div class="font-bold text-slate-800 text-lg">${i + 1}. ${p.name || ''}</div>
      ${p.owner ? `<span class="text-xs text-slate-500 whitespace-nowrap">Owner: <strong>${p.owner}</strong></span>` : ''}
    </div>
    ${p.business_goal ? `<div class="mb-2"><span class="badge">Negócio</span> <span class="text-sm text-slate-600">${p.business_goal}</span></div>` : ''}
    ${p.objective ? `<div class="text-sm mb-2"><strong>Objetivo:</strong> ${p.objective}</div>` : ''}
    <div class="text-xs uppercase font-bold text-slate-400 mt-2 mb-1">Key Results</div>
    ${krs}
    ${inits}
    ${p.metric ? `<div class="mt-2 text-xs text-slate-500">Métrica-chave: <strong>${p.metric}</strong></div>` : ''}
  </div>`;
}).join('');

if (DATA.risks.length) { document.getElementById('risks-card').style.display = ''; document.getElementById('risks').innerHTML = DATA.risks.map(r => `<li>${r}</li>`).join(''); }
if (DATA.dependencies.length) { document.getElementById('deps-card').style.display = ''; document.getElementById('deps').innerHTML = DATA.dependencies.map(r => `<li>${r}</li>`).join(''); }
...
if (DATA.roadmap.length) {
  ...
  document.getElementById('roadmap').innerHTML = DATA.roadmap.map(q =>
    `<div class="q"><div class="q-name">${q.quarter || ''}</div><ul class="text-sm">${(q.milestones || []).map(m => `<li>${m}</li>`).join('')}</ul></div>`).join('');
}
</script>
</body></html>
"""
