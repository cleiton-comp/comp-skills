...
document.getElementById('scen-tbody').innerHTML = order.map(k => {
  ...
  return `<tr ${bold}><td>${s.name}</td><td>${Math.round(s.target_total)}</td><td>${Math.round(s.total_net_hires)}</td><td>${fmt(s.incr_annual_runrate)}</td><td>${fmt(s.cumulative_cost_horizon)}</td></tr>`;
}).join('');
...
document.getElementById('roadmap-tbody').innerHTML = base.functions.map(fp => {
  let row = `<td><strong>${fp.function}</strong></td><td>${fp.start_hc}</td>`;
