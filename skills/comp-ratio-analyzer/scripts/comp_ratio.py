...
document.getElementById('lvl-tbody').innerHTML = DATA.by_level.map(l =>
  `<tr><td><strong>${l.level}</strong></td><td>${l.total}</td><td>${l.avg_ratio}</td><td>${l.min_ratio}</td><td>${l.max_ratio}</td></tr>`).join('');

document.getElementById('under-tbody').innerHTML = DATA.under_outliers.map(c =>
  `<tr><td>${c.name}</td><td>${c.level}</td><td>${fmt(c.salary)}</td><td>${fmt(c.mid)}</td><td><span class="pill ${distClass[c.class]}">${c.ratio}</span></td><td class="text-rose-600">${fmt(c.delta_to_mid)}</td></tr>`).join('');
document.getElementById('over-tbody').innerHTML = DATA.over_outliers.map(c =>
  `<tr><td>${c.name}</td><td>${c.level}</td><td>${fmt(c.salary)}</td><td>${fmt(c.mid)}</td><td><span class="pill ${distClass[c.class]}">${c.ratio}</span></td><td class="text-emerald-600">+${fmt(c.delta_to_mid)}</td></tr>`).join('');
</script>
</body></html>
"""
