...
if (DATA.rising_negative.length > 0) {
  ...
  document.getElementById('rising-list').innerHTML = DATA.rising_negative.map(r =>
    `<span class="pill sent-negativo">${r}</span>`).join('');
}
...
document.getElementById('themes-list').innerHTML = DATA.themes.map(t => {
  ...
  const quotes = (t.quotes || []).map(q => `<div class="quote">${q}</div>`).join('');
  const actions = (t.recommended_actions || []).length
    ? `<div class="mt-2 text-sm"><strong>Ações:</strong><ul class="list-disc pl-5 mt-1">${t.recommended_actions.map(a => `<li>${a}</li>`).join('')}</ul></div>`
    : '';
  return `<div>
    <div class="flex justify-between items-center mb-1">
      <div class="font-bold text-slate-800">${t.name} <span class="text-slate-400 font-normal">· ${t.count} (${t.pct}%)</span></div>
      <span class="pill ${sc}">${t.sentiment}</span>
    </div>
...
if (DATA.segments.length > 0) {
  ...
  document.getElementById('seg-tbody').innerHTML = DATA.segments.map(s =>
    `<tr><td><strong>${s.segment}</strong></td><td>${s.theme}</td><td>${s.count}</td></tr>`).join('');
}
