...
document.getElementById('area-tbody').innerHTML = DATA.headcount.by_area.map(a =>
  `<tr><td><strong>${a.name}</strong></td><td>${a.hc}</td><td>${pct(a.hc, s.headcount)}</td></tr>`).join('');
document.getElementById('level-tbody').innerHTML = DATA.headcount.by_level.map(l =>
  `<tr><td><strong>${l.name}</strong></td><td>${l.hc}</td><td>${pct(l.hc, s.headcount)}</td></tr>`).join('');
...
if (DATA.span) {
  document.getElementById('span-tbody').innerHTML = Object.entries(DATA.span.distribution).map(([k, v]) =>
    `<tr><td>${k} reportes</td><td>${v}</td></tr>`).join('');
...
if (DATA.trend && DATA.trend.length) {
  ...
  document.getElementById('trend-tbody').innerHTML = DATA.trend.map(m =>
    `<tr><td><strong>${m.month}</strong></td><td>${m.hire}</td><td class="text-rose-600">${m.exit}</td><td class="text-emerald-600">${m.promotion}</td></tr>`).join('');
}
</script>
</body></html>
"""
