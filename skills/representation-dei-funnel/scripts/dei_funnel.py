...
def analyze(roster, cols, events, ecols, dim_col, dim_label):
    ...
    if not overall or not overall["available"]:
        insights.append(f"Não foi possível ler o recorte '{dim_label}' no roster — verifique a coluna.")
    else:
        if leadership_gap:
            worst = leadership_gap[0]
            if worst["gap"] <= -5:
                insights.append(f"Grupo <strong>{worst['group']}</strong> tem gap de liderança de <strong>{worst['gap']} p.p.</strong> ({worst['overall_pct']}% geral vs {worst['leadership_pct'] or 0}% na liderança).")
        if leaks:
...
document.getElementById('legend').innerHTML = DATA.summary.groups.map(g =>
  `<span class="legend"><span class="swatch" style="background:${colorFor[g]}"></span>${g}</span>`).join('');

document.getElementById('stages').innerHTML = DATA.stages.map(s => {
  if (!s.available) return `<div><div class="text-sm font-semibold text-slate-700">${s.stage}</div><div class="text-xs text-slate-400">sem dados</div></div>`;
  const segs = DATA.summary.groups.filter(g => s.mix[g] !== undefined).map(g =>
    `<div class="seg" style="width:${s.mix[g]}%;background:${colorFor[g]}" title="${g}: ${s.mix[g]}%">${s.mix[g] >= 8 ? s.mix[g]+'%' : ''}</div>`).join('');
  const supp = s.suppressed > 0 ? ` <span class="text-xs text-slate-400">(+${s.suppressed} omitidos)</span>` : '';
  return `<div><div class="text-sm font-semibold text-slate-700 mb-1">${s.stage} <span class="text-slate-400 font-normal">— n=${s.n}</span>${supp}</div><div class="bar">${segs}</div></div>`;
}).join('');
...
if (DATA.leaks.length > 0) {
  ...
  document.getElementById('leaks').innerHTML = DATA.leaks.map(l =>
    `<div class="leak"><strong>${l.group}</strong><ul class="list-disc ml-5 mt-1 text-sm text-slate-700">${l.notes.map(n => `<li>${n}</li>`).join('')}</ul></div>`).join('');
}
...
if (DATA.leadership_gap.length > 0) {
  ...
  document.getElementById('gap-tbody').innerHTML = DATA.leadership_gap.map(r =>
    `<tr><td><strong>${r.group}</strong></td><td>${r.overall_pct}%</td><td>${r.leadership_pct === null ? '—' : r.leadership_pct+'%'}</td><td class="${r.gap < 0 ? 'text-rose-600' : 'text-emerald-600'} font-semibold">${r.gap > 0 ? '+' : ''}${r.gap}</td></tr>`).join('');
}

if (DATA.rates.length > 0) {
  document.getElementById('rates-card').style.display = '';
  document.getElementById('rates-tbody').innerHTML = DATA.rates.map(r =>
    `<tr><td><strong>${r.group}</strong></td><td>${r.hc}</td><td>${r.promo_rate === null ? '—' : r.promo_rate+'%'}</td><td>${r.exit_rate === null ? '—' : r.exit_rate+'%'}</td></tr>`).join('');
}
</script>
</body></html>
"""
