...
def render_html(data):
    ...
    if metrics:
        ...
        for m in metrics:
            ...
            parts.append(f'<div class="metric"><div><div class="metric-name">{_esc(m.get("name"))}</div>{f"<div class=\"text-sm text-slate-600\">{_esc(m.get(\"context\"))}</div>" if m.get("context") else ""}</div><div class="metric-val {cls}">{arrow} {_esc(m.get("current"))}{prev}</div></div>')
        parts.append("</div>")
