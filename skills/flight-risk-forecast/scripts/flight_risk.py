...
def analyze(roster, cols):
    ...
    if total == 0:
        ...
        if levers:
            top = levers[0]
            insights.append(f"Maior driver agregado: <strong>{top['driver']}</strong> afetando {top['affected']} pessoa(s) em risco.")
...
document.getElementById('risk-tbody').innerHTML = DATA.at_risk_list.map(s => {
  const fac = s.factors.map(f => `<span class="fpill">${f.factor} +${f.points}</span>`).join(' ');
  const reg = s.regretted_risk ? ' <span class="pill band-critical">lamentável</span>' : '';
  return `<tr>
    <td><strong>${s.name}</strong>${reg}</td>
    <td>${s.area}</td><td>${s.manager}</td>
...
document.getElementById('area-tbody').innerHTML = DATA.hottest_areas.map(a =>
  `<tr><td><strong>${a.name}</strong></td><td>${a.hc}</td><td>${a.avg_score}</td><td class="text-orange-600">${a.at_risk}</td></tr>`).join('');

if (DATA.hottest_managers.length > 0) {
  document.getElementById('mgr-tbody').innerHTML = DATA.hottest_managers.map(m =>
    `<tr><td><strong>${m.name}</strong></td><td>${m.hc}</td><td>${m.avg_score}</td><td class="text-orange-600">${m.at_risk}</td></tr>`).join('');
} else {
...
if (DATA.levers.length > 0) {
  document.getElementById('lever-tbody').innerHTML = DATA.levers.map(l =>
    `<tr><td><strong>${l.driver}</strong></td><td>${l.affected}</td><td>${l.lever}</td></tr>`).join('');
} else {
