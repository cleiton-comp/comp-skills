...
document.getElementById('ben-tbody').innerHTML = DATA.benefit_lines.map(l =>
  `<tr><td><strong>${l.label}</strong></td><td class="text-slate-500">${l.detail}</td><td class="pos">${fmt(l.annual)}</td></tr>`).join('')
...
document.getElementById('sens-tbody').innerHTML = DATA.sensitivity.map(s => {
  ...
  return `<tr ${bold}><td>${s.name}</td><td>${s.mult}x</td><td>${fmt(s.annual_benefit)}</td><td>${roi}</td><td>${pb}</td><td class="${cls}">${fmt(s.net_3yr)}</td></tr>`;
}).join('');
