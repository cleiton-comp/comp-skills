...
document.getElementById('areas-tbody').innerHTML = DATA.areas.map(a =>
  `<tr><td><strong>${a.area}</strong></td><td>${a.headcount}</td><td>${a.managers}</td><td>${a.avg_span}</td><td>${a.ai_agents_total}</td><td>${a.automation_avg_pct}%</td></tr>`).join('');

document.getElementById('top-tbody').innerHTML = DATA.top_spans.map(m =>
  `<tr><td>${m.name}</td><td>${m.area}</td><td><strong>${m.directs}</strong></td><td>${m.ai_agents}</td><td>${m.automation_pct}%</td><td>${m.adjusted_span}</td><td><span class="pill ${soiColors[m.soi_category]}">${soiLabels[m.soi_category]}</span></td></tr>`).join('');
</script>
</body></html>
"""
