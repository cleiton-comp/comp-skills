...
def analyze(rows, cols):
    ...
    if area_ranked:
        ...
        if top_area["pct"] > overall_pct * 1.5 and top_area["regretted"] >= 3:
            insights.append(f"Área <strong>{top_area['key']}</strong> tem {top_area['pct']}% regretted ({top_area['regretted']}/{top_area['total']}) — {round(top_area['pct']/overall_pct, 1)}x a média da empresa.")
    ...
    if mgr_ranked:
        ...
        if top_mgr["regretted"] >= 3:
            insights.append(f"Gestor <strong>{top_mgr['key']}</strong> concentra {top_mgr['regretted']} desligamentos regretted ({top_mgr['pct']}% do time dele) — pattern pra investigar.")
...
document.getElementById('area-tbody').innerHTML = DATA.by_area.map(r =>
  `<tr><td><strong>${r.key}</strong></td><td>${r.total}</td><td>${r.regretted}</td><td class="${pctClass(r.pct)}">${r.pct}%</td></tr>`).join('');
document.getElementById('tenure-tbody').innerHTML = DATA.by_tenure.map(r => {
  ...
  return `<tr><td><strong>${r.key}</strong></td><td>${r.total}</td><td>${r.regretted}</td><td class="${pctClass(pct)}">${pct}%</td></tr>`;
}).join('');
...
document.getElementById('mgr-tbody').innerHTML = DATA.by_manager_top10.map(r =>
  `<tr><td>${r.key}</td><td>${r.total}</td><td>${r.regretted}</td><td class="${pctClass(r.pct)}">${r.pct}%</td></tr>`).join('');
...
if (DATA.top_reasons.length > 0) {
  ...
  document.getElementById('reasons-tbody').innerHTML = DATA.top_reasons.map(r =>
    `<tr><td>${r.reason}</td><td><strong>${r.count}</strong></td></tr>`).join('');
}
</script>
</body></html>
"""
