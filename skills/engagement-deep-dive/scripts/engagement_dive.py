...
def analyze(rows, cols):
    ...
    if area_rows and overall_score:
        ...
        if worst_area["avg_score"] < overall_score - 1.0:
            insights.append(f"Área <strong>{worst_area['segment']}</strong> ({worst_area['avg_score']}) muito abaixo da média ({overall_score}). Foco prioritário.")
    if tenure_rows:
    ...
    if bottom_managers and len(bottom_managers) > 0:
        ...
        if overall_score and worst_mgr["avg_score"] < overall_score - 1.5:
            insights.append(f"Gestor <strong>{worst_mgr['manager']}</strong> com média {worst_mgr['avg_score']} ({worst_mgr['n']} respostas) — abaixo significativamente da empresa.")
...
document.getElementById('area-tbody').innerHTML = DATA.by_area.map(r => `<tr><td><strong>${r.segment}</strong></td><td>${r.n}</td><td class="${scoreCls(r.avg_score)}">${r.avg_score}</td><td>${r.min_score}</td><td>${r.max_score}</td></tr>`).join('');
document.getElementById('tenure-tbody').innerHTML = DATA.by_tenure.map(r => `<tr><td><strong>${r.segment}</strong></td><td>${r.n}</td><td class="${scoreCls(r.avg_score)}">${r.avg_score}</td><td>${r.min_score}</td><td>${r.max_score}</td></tr>`).join('');
document.getElementById('lvl-tbody').innerHTML = DATA.by_level.map(r => `<tr><td><strong>${r.segment}</strong></td><td>${r.n}</td><td class="${scoreCls(r.avg_score)}">${r.avg_score}</td><td>${r.min_score}</td><td>${r.max_score}</td></tr>`).join('');
document.getElementById('mgr-tbody').innerHTML = DATA.bottom_managers.map(r => `<tr><td>${r.manager}</td><td>${r.n}</td><td class="${scoreCls(r.avg_score)}">${r.avg_score}</td></tr>`).join('');
</script>
</body></html>
"""
