...
def analyze(promotions, eligible, cols_p, cols_e):
    ...
    if disparities:
        ...
        if abs(worst["ratio_f_to_m"] - 1) > 0.3:
            insights.append(f"Área <strong>{worst['area']}</strong> com maior disparidade: ratio F/M = {worst['ratio_f_to_m']} (F {worst['f_rate']}% vs M {worst['m_rate']}%).")
...
if (DATA.summary.has_eligible) {
  ...
  document.getElementById('rates-tbody').innerHTML = Object.entries(DATA.rates).map(([g, r]) =>
    `<tr><td><strong>${g}</strong></td><td>${r.eligible}</td><td>${r.promoted}</td><td>${r.rate_pct}%</td></tr>`).join('');
  if (DATA.disparities.length > 0) {
    ...
    document.getElementById('disp-tbody').innerHTML = DATA.disparities.map(d =>
      `<tr><td><strong>${d.area}</strong></td><td>${d.f_eligible}</td><td>${d.f_promoted}</td><td>${d.f_rate}%</td><td>${d.m_eligible}</td><td>${d.m_promoted}</td><td>${d.m_rate}%</td><td><strong>${d.ratio_f_to_m}</strong></td></tr>`).join('');
  }
}

document.getElementById('lvl-tbody').innerHTML = DATA.by_levels.map(l =>
  `<tr><td>${l.transition}</td><td><strong>${l.count}</strong></td></tr>`).join('');
</script>
</body></html>
"""
