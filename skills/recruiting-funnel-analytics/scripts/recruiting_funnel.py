...
def analyze(rows, cols):
    ...
    if total == 0:
        ...
        if bottleneck:
            insights.append(f"Gargalo no funil: <strong>{bottleneck['from_label']} → {bottleneck['label']}</strong> com apenas <strong>{bottleneck['passthrough']}%</strong> de pass-through.")
        ...
        if by_source:
            best = by_source[0]
            insights.append(f"Fonte com mais contratações: <strong>{best['source']}</strong> ({best['hired']} hires, {best['conversion']}% de conversão).")
...
document.getElementById('funnel').innerHTML = DATA.funnel.map(f => {
  ...
  return `<div class="flex items-center gap-3">
    <div style="width:${w}%"><div class="fbar"><span>${f.label}</span><span>${f.count}</span></div></div>
    ${ptxt}
  </div>`;
}).join('');
...
if (DATA.bottleneck) {
  ...
  document.getElementById('bottleneck-text').innerHTML = `Menor pass-through em <strong>${DATA.bottleneck.from_label} → ${DATA.bottleneck.label}</strong>: apenas <strong>${DATA.bottleneck.passthrough}%</strong> avançam. Priorize melhorias nesse estágio.`;
}
...
if (DATA.by_source.length > 0) {
  ...
  document.getElementById('source-tbody').innerHTML = DATA.by_source.map(s =>
    `<tr><td><strong>${s.source}</strong></td><td>${s.applied}</td><td>${s.hired}</td><td>${s.conversion}%</td></tr>`).join('');
}
...
if (DATA.decline_reasons.length > 0) {
  ...
  document.getElementById('decline-tbody').innerHTML = DATA.decline_reasons.map(d =>
    `<tr><td>${d.reason}</td><td>${d.count}</td></tr>`).join('');
}
</script>
</body></html>
"""
