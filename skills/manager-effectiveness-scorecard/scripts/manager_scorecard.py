...
document.getElementById('rank-tbody').innerHTML = DATA.ranked.map((s, i) =>
  `<tr><td><strong>${i+1}</strong></td><td><strong>${s.manager}</strong></td><td>${bandPill(s.band)}</td><td><strong>${num(s.score)}</strong></td><td>${s.span}</td><td>${pct(s.team_engagement)}</td><td>${pct(s.regretted_attrition !== null ? s.regretted_attrition : s.attrition)}</td><td>${pct(s.promo_rate)}</td><td>${flagsHtml(s.flags)}</td></tr>`).join('');
...
if (DATA.small.length > 0) {
  ...
  document.getElementById('small-tbody').innerHTML = DATA.small.map(s =>
    `<tr><td><strong>${s.manager}</strong></td><td>${s.span}</td><td>${pct(s.team_engagement)}</td><td>${pct(s.regretted_attrition !== null ? s.regretted_attrition : s.attrition)}</td><td>${pct(s.promo_rate)}</td><td>${flagsHtml(s.flags)}</td></tr>`).join('');
}
</script>
</body></html>
"""
